/*****************************************************************************************/
* Program Name  : LBPLOTH_report_Central Lab - Hematology Plots.sas
* Project       : BOS-580-201
* Programmer    : Mark Woodruff
* Creation Date : 2022-03-17
* Description   : report LBCPLOTH domain
*
* Revision History
...
data domain_data;
	...
	where subnum="&ptn." and lbstresn>.z and lbcat='Hematology';
run;
...
%macro report_domain;
	...
	ods proclabel='Central Lab - Hematology Plots';
	ods graphics / height=&test_height.px width=1150px imagename="lbhgraph_&ptn." imagemap=on noborder;

	%if %eval(&exist_lbdy.)>0 %then %do;
		proc sgpanel data=domain_data noautolegend nocycleattrs pad=0;
			title 'Central Lab - Hematology Plots';
	...
	%end;
	%else %do;
		%if &exist_lbdat.=0 %then %do; title "Central Lab - Hematology Plots add-no-data"; %end;
			%else %if &exist_lbdat.=1 and &exist_rfstdt.=0 %then %do; title "Central Lab - Hematology Plots add-no-dosing-data"; %end;
		data _null_;
			a=1;
