/*****************************************************************************************/
* Program Name  : LBPLOTO_report_Central Lab - Other Plots.sas
* Project       : BOS-580-201
* Programmer    : Mark Woodruff
* Creation Date : 2022-03-17
* Description   : report LBCPLOTO domain
*
* Revision History
...
data domain_data;
	...
	where subnum="&ptn." and lbstresn>.z and other_plot=1;
run;
...
%macro report_domain;
	...
	ods proclabel='Central Lab - Other Plots';
	ods graphics / height=&test_height.px width=1150px imagename="lbograph_&ptn." imagemap=on noborder;

	%if %eval(&exist_lbdy.)>0 %then %do;
		proc sgpanel data=domain_data noautolegend nocycleattrs pad=0;
			title 'Central Lab - Other Plots';
	...
	%end;
	%else %do;
		%if &exist_lbdat.=0 %then %do; title "Central Lab - Other Plots add-no-data"; %end;
			%else %if &exist_lbdat.=1 and &exist_rfstdt.=0 %then %do; title "Central Lab - Other Plots add-no-dosing-data"; %end;
		data _null_;
			a=1;
